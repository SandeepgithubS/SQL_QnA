...
INSERT INTO walmart (invoiceno, stockcode, description, quantity, invoicedate, unitprice, customerid, country) VALUES
...
select 
     ...
     description,
     round(sum(quantity * unitprice),0) as rev,
     row_number() over(partition by month(invoicedate) order by round(sum(quantity * unitprice),0) desc) as rk
from walmart
group by 1,2)
