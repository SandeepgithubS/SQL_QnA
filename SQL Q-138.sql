...
select 
     distinct a.category,
     count(b.order_id) over(partition by a.category) as cat,
     count(b.order_id) over() as totcat,
     round( count(b.order_id) over(partition by a.category) /
     count(b.order_id) over() *100,1) as "%"
from amazon_products as a join return_records as b
