...
/* Question: Identify the top 3 posts with the highest engagement (likes + comments) for each user on a Facebook page. Display 
...
select 
      ...
      dense_rank() over(partition by user_id order by (likes + comments)  desc) as rk
from fb_posts )
